              ...
              {!loading && !error && products.length > 0 && (
                <ul className="flex flex-col gap-3">
                  {products.map((prod) => (
                    <li
                      key={prod.id}
                      className="group flex items-center justify-between p-4 rounded-xl border border-slate-100 dark:border-neutral-800/80 hover:border-slate-200 dark:hover:border-neutral-700 bg-white dark:bg-zinc-900 transition-all duration-150"
                    >
                      <div className="flex flex-col gap-0.5">
                        <span className="text-sm font-bold text-slate-800 dark:text-zinc-200">
                          {prod.product || "Unnamed Product"}
                        </span>
