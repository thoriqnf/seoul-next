      ...
      <main className="flex-1 w-full max-w-md mx-auto px-4 py-24 flex flex-col justify-center font-sans">
        <div className="bg-white dark:bg-zinc-950/40 border border-slate-200/65 dark:border-neutral-800/85 rounded-2xl p-8 shadow-sm text-center">
          ...
          <p className="text-xs text-slate-500 dark:text-zinc-400 mb-6 leading-relaxed">
            You do not have the required permissions to view this dashboard page. Please sign in with a authorized Store Staff account.
          </p>

          <div className="flex flex-col gap-3">
            <Link
              href="/login"
              className="w-full inline-flex items-center justify-center h-10 rounded-full bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold transition-colors shadow-sm cursor-pointer"
            >
            ...
            <Link
              href="/"
              className="text-xs text-indigo-650 dark:text-indigo-400 font-bold hover:underline transition-colors mt-2"
            >
              Return to Toyiverse Shop
            </Link>
          </div>
        </div>
      </main>
